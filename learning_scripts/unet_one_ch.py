...
with open("/home/dataset/config_unet_1ch.yaml", 'r') as stream:
...
log_txt_name = "/home/log_unet/log_unet_one_ch_" + now_time + ".txt"
...
a = wandb.init(
    ...
    project="segmentation-one-channel"
)
...
class SegmentationDataset(Dataset):
    def __init__(self, input_dir, is_train, transform=None):
        ...
        if is_train == True:
            path = input_dir + '/images_1/train'
            self.images = os.listdir(path)
        else:
            path = input_dir + '/images_1/val'
            self.images = os.listdir(path)
    ...
    def __getitem__(self, index):
        if self.is_train == True:
            img_path = self.input_dir +'/images_1/train/'+ self.images[index]
            mask_path = self.input_dir +'/masks/train/'+ self.images[index][:-4] + 'png'
        else:
            img_path = self.input_dir +'/images_1/val/'+ self.images[index]
            mask_path = self.input_dir +'/masks/val/'+ self.images[index][:-4] + 'png'
...
# In[13]:
aux_params = dict(dropout=0.1,
                  classes=data_loaded['input_classes'])
model = smp.Unet(encoder_name='resnet18', classes=data_loaded['input_classes'], in_channels=data_loaded['input_channels'], activation=None, aux_params=aux_params).to(DEVICE)
